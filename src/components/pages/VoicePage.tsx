...
export default function VoicePage() {
  ...
  useEffect(() => {
    ...
    const speechLang = languageCodeToSpeechLang[language.code];
    if (!speechLang) {
      setError(`Speech recognition not supported for ${language.name}. Please try English or Hindi.`);
      return;
    }
  ...
  const playResponse = () => {
    ...
    const speechLang = languageCodeToSpeechLang[language.code] || 'en-US';
    const utterance = new SpeechSynthesisUtterance(response);
    ...
    const preferredVoice = voices.find(voice =>
      voice.lang === speechLang || voice.lang.startsWith(speechLang.split('-')[0])
    );
