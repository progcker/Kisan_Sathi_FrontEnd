...
export default function TextPage() {
  ...
  const getAIResponse = async (userMessage: string): Promise<string> => {
    try {
      ...
      return await geminiApi.askQuestion(userMessage, language?.code || 'en', userInfo?.location || '');
    } catch (error) {
      console.error('AI Response Error:', error);
      const fallbackMessages: Record<string, string> = {
        en: `${userInfo?.name || 'User'}, sorry, there's currently an issue with the AI service. Please try again later. For emergencies, contact your local agricultural expert.`,
      };
      return fallbackMessages[language?.code || 'en'] || fallbackMessages.en;
    }
  };
